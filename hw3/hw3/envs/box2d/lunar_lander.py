...
import numpy as np
from Box2D import (
    b2ContactListener, b2EdgeShape, b2PolygonShape, b2FixtureDef, b2RevoluteJointDef,
    b2CircleShape, b2World
)
...
class LunarLander(gym.Env):
    ...
    def _reset(self):
        ...
        self.moon = self.world.CreateStaticBody(shapes=b2EdgeShape(vertices=[(0, 0), (W, 0)]))
        self.sky_polys = []
        ...
        self.lander = self.world.CreateDynamicBody(
            ...
            fixtures=b2FixtureDef(
                shape=b2PolygonShape(vertices=[(x / SCALE, y / SCALE) for x, y in LANDER_POLY]),
                density=5.0,
                friction=0.1,
                categoryBits=0x0010,
    ...
    def _step(self, action):
        ...
        if action[0] > 0.0:
            ...
            p.ApplyLinearImpulse(
                (ox * MAIN_ENGINE_POWER * m_power, oy * MAIN_ENGINE_POWER * m_power), impulse_pos,
                True
            )
        ...
        if np.abs(action[1]) > 0.5:
            ...
            p.ApplyLinearImpulse(
                (ox * SIDE_ENGINE_POWER * s_power, oy * SIDE_ENGINE_POWER * s_power), impulse_pos,
                True
            )
            self.lander.ApplyLinearImpulse(
                (-ox * SIDE_ENGINE_POWER * s_power, -oy * SIDE_ENGINE_POWER * s_power), impulse_pos,
    ...
    def _render(self, mode='rgb_array', close=False):
        ...
        for x in [self.helipad_x1, self.helipad_x2]:
            ...
            self.viewer.draw_polygon(
                [(x, flagy2), (x, flagy2 - 10 / SCALE), (x + 25 / SCALE, flagy2 - 5 / SCALE)],
                color=(0.8, 0.8, 0)
            )
        ...
        self.viewer.draw_polyline(
            [(0, 0.05 * VIEWPORT_H / SCALE),
                (clock_prog * VIEWPORT_W / SCALE, 0.05 * VIEWPORT_H / SCALE)], color=(255, 0, 0),
            linewidth=5
        )
...
def heuristic(env, s):
    ...
    if s[6] or s[7]:  # legs have contact
        angle_todo = 0
        hover_todo = -(
        s[3]) * 0.5  # override to reduce fall speed, that's all we need after contact
